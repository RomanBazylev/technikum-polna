...
export default function TeacherDirectory({ subjects }: Props) {
  ...
  return (
    <section className="rounded-card border border-[var(--color-line)] p-4">
      ...
      <p className="mt-2 text-label text-[var(--color-muted)]">
      ...
      {teachers.length === 0 ? (
        <p className="mt-4 text-label text-[var(--color-muted)]">
            ...
            <li key={entry.id} className="rounded-lg border border-[var(--color-line)] p-3">
              <div className="grid grid-cols-1 gap-2 text-label sm:grid-cols-2">
                <label className="block">
                  <span className="text-micro text-[var(--color-muted)]">Imię i nazwisko · Имя</span>
                  <input
                    value={entry.name}
                ...
                <label className="block">
                  <span className="text-micro text-[var(--color-muted)]">Przedmiot · Предмет</span>
                  <select
                ...
                <label className="block">
                  <span className="text-micro text-[var(--color-muted)]">Sala · Кабинет</span>
                  <input
                    value={entry.room ?? ''}
                    placeholder="np. 204"
                    onInput={(event) =>
                ...
                <label className="block">
                  <span className="text-micro text-[var(--color-muted)]">Poczta szkolna · Школьная почта</span>
                  <input
                    type="email"
                    value={entry.email ?? ''}
              ...
              <div className="mt-2 flex items-center justify-between">
                <span className="text-micro text-[var(--color-faint)]">{nameOf(entry.subjectId)}</span>
                <button
                  type="button"
      ...
      <button
        ...
        onClick={add}
        disabled={!ready}
        className="mt-4 rounded-lg border border-[var(--color-line)] px-3 py-2 text-label disabled:opacity-40"
      >
